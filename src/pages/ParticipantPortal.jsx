    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50">
      ...
      <div className="max-w-2xl mx-auto p-4 py-6 space-y-4">
        ...
        {activeTab === "implementation" && (() => {
          if (ipLoading) return <div className="flex items-center justify-center h-40"><Loader2 className="animate-spin text-primary" size={28} /></div>;
          if (ipView === "form" && ipForm) {
            const ipSetF = (k, v) => setIpForm(p => ({ ...p, [k]: v }));
            return (
              <div className="space-y-6">
                <div className="flex justify-between items-center"><div><button onClick={() => { setIpView("list"); setIpEditingId(null); }} className="text-blue-600 text-sm font-bold hover:underline">← Back</button><h2 className="text-2xl font-black mt-1">{ipEditingId ? "Edit" : "New"} Implementation Program</h2></div><Button onClick={ipSave} disabled={!ipForm.participant_name || ipSaving} className="rounded-xl font-bold gap-2">{ipSaving ? <Loader2 size={15} className="animate-spin" /> : <Save size={15} />} {ipEditingId ? "Update" : "Create"} Program</Button></div>
            ...
            return (
              <div className="space-y-6">
                <div className="flex flex-col sm:flex-row justify-between items-start gap-4">
                  ...
                  <div className="flex gap-2 flex-wrap"><span className={`px-3 py-1 rounded-full text-xs font-black ${ipSelected.status === "Active" ? "bg-emerald-100 text-emerald-700" : "bg-slate-100 text-slate-600"}`}>{ipSelected.status}</span><Button variant="outline" size="sm" onClick={() => ipEditProgram(ipSelected)} className="rounded-lg text-xs gap-1"><Pencil size={12} /> Edit</Button><Button variant="ghost" size="sm" onClick={() => ipDelete(ipSelected.id, ipSelected.participant_name)} className="rounded-lg text-xs gap-1 text-red-600 hover:text-red-700"><Trash2 size={12} /> Delete</Button></div>
                </div>
                ...
                <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
                  <div className="xl:col-span-2 space-y-4">
                    <div className="bg-white border border-slate-200 rounded-3xl overflow-hidden"><button onClick={() => ipToggleSection("phases")} className="w-full flex justify-between items-center px-6 py-4 hover:bg-slate-50 transition-colors"><h3 className="font-black text-base flex items-center gap-2"><BarChart3 size={16} className="text-blue-600" /> Implementation Phases</h3>{ipExpandedSections.phases ? <ChevronUp size={16} /> : <ChevronDown size={16} />}</button>{ipExpandedSections.phases && (<div className="px-6 pb-6 space-y-3">{(ipSelected.phases || []).map((ph, i) => (<div key={i} className={`border rounded-2xl overflow-hidden ${ph.completed ? "border-emerald-200" : "border-slate-200"}`}><div className={`px-5 py-3 flex items-center justify-between ${ph.completed ? "bg-emerald-50" : "bg-slate-100"}`}><div className="flex items-center gap-3"><span className={`text-white text-[10px] font-black px-2 py-0.5 rounded-full ${IP_PHASE_COLORS[i] || "bg-slate-500"}`}>Phase {ph.phase_number}</span><div><p className="font-black text-sm">{ph.name}</p><p className="text-[10px] text-slate-500">{ph.weeks} · Support: {ph.support_level} · Role: {ph.worker_role}</p></div></div><button onClick={() => ipTogglePhase(i)} className={ph.completed ? "text-emerald-600" : "text-slate-400 hover:text-emerald-600"}>{ph.completed ? <CheckCircle size={20} /> : <Circle size={20} />}</button></div>{ph.goal && (<div className="px-5 py-3 text-sm text-slate-500"><span className="font-bold text-slate-800">Goal: </span>{ph.goal}</div>)}</div>))}</div>)}</div>
                    <div className="bg-white border border-slate-200 rounded-3xl overflow-hidden"><button onClick={() => ipToggleSection("logs")} className="w-full flex justify-between items-center px-6 py-4 hover:bg-slate-50 transition-colors"><h3 className="font-black text-base flex items-center gap-2"><ClipboardList size={16} className="text-blue-600" /> Session Logs ({(ipSelected.session_logs || []).length})</h3>{ipExpandedSections.logs ? <ChevronUp size={16} /> : <ChevronDown size={16} />}</button>{ipExpandedSections.logs && (<div className="px-6 pb-6 space-y-4"><div className="bg-blue-50 border border-blue-200 rounded-2xl p-4 space-y-3"><p className="text-xs font-black text-blue-600 uppercase tracking-widest">Log New Session</p><div className="grid grid-cols-2 sm:grid-cols-3 gap-3"><div><Label className="text-[10px]">Date</Label><Input type="date" value={ipNewLog.date} onChange={e => setIpNewLog(p => ({ ...p, date: e.target.value }))} className="h-8 text-xs mt-0.5" /></div><div><Label className="text-[10px]">Phase</Label><Select value={ipNewLog.phase} onValueChange={v => setIpNewLog(p => ({ ...p, phase: v }))}><SelectTrigger className="h-8 text-xs mt-0.5"><SelectValue /></SelectTrigger><SelectContent>{(ipSelected.phases || []).map(ph => <SelectItem key={ph.phase_number} value={`Phase ${ph.phase_number}`}>Phase {ph.phase_number} — {ph.name}</SelectItem>)}</SelectContent></Select></div><div><Label className="text-[10px]">Support Level Used</Label><Input value={ipNewLog.support_level_used} onChange={e => setIpNewLog(p => ({ ...p, support_level_used: e.target.value }))} placeholder="e.g. 70%" className="h-8 text-xs mt-0.5" /></div><div><Label className="text-[10px]">Logged By</Label><Input value={ipNewLog.logged_by} onChange={e => setIpNewLog(p => ({ ...p, logged_by: e.target.value }))} placeholder="Staff name" className="h-8 text-xs mt-0.5" /></div></div><div><Label className="text-[10px]">Skills Practiced</Label><Textarea value={ipNewLog.skills_practiced} onChange={e => setIpNewLog(p => ({ ...p, skills_practiced: e.target.value }))} placeholder="What skills did the participant practice today?" className="text-xs min-h-[50px] mt-0.5" /></div><div><Label className="text-[10px]">Participant Response</Label><Textarea value={ipNewLog.participant_response} onChange={e => setIpNewLog(p => ({ ...p, participant_response: e.target.value }))} placeholder="How did the participant respond?" className="text-xs min-h-[50px] mt-0.5" /></div><div className="grid grid-cols-2 gap-3"><div><Label className="text-[10px]">Incidents / Concerns</Label><Input value={ipNewLog.incidents} onChange={e => setIpNewLog(p => ({ ...p, incidents: e.target.value }))} placeholder="None" className="h-8 text-xs mt-0.5" /></div><div><Label className="text-[10px]">Next Session Focus</Label><Input value={ipNewLog.next_session_focus} onChange={e => setIpNewLog(p => ({ ...p, next_session_focus: e.target.value }))} className="h-8 text-xs mt-0.5" /></div></div><Button onClick={ipAddLog} size="sm" className="rounded-xl gap-1 font-bold"><Save size={13} /> Save Session Log</Button></div>{(ipSelected.session_logs || []).length === 0 ? <p className="text-sm text-slate-400 italic text-center py-4">No sessions logged yet.</p> : (<div className="space-y-3">{[...(ipSelected.session_logs || [])].reverse().map((log, i) => (<div key={i} className="bg-slate-100 rounded-2xl p-4"><div className="flex justify-between items-start mb-2"><div className="flex items-center gap-2"><span className="text-xs font-black text-slate-800">{log.date}</span><span className="text-[10px] bg-blue-100 text-blue-700 px-2 py-0.5 rounded-full font-bold">{log.phase}</span>{log.support_level_used && <span className="text-[10px] bg-amber-100 text-amber-700 px-2 py-0.5 rounded-full font-bold">{log.support_level_used} support</span>}</div><span className="text-[10px] text-slate-500">{log.logged_by}</span></div>{log.skills_practiced && <p className="text-xs text-slate-800 mb-1"><span className="font-bold">Skills: </span>{log.skills_practiced}</p>}{log.participant_response && <p className="text-xs text-slate-500 mb-1"><span className="font-bold text-slate-700">Response: </span>{log.participant_response}</p>}{log.next_session_focus && <p className="text-xs text-slate-500"><span className="font-bold text-slate-700">Next: </span>{log.next_session_focus}</p>}</div>))}</div>)}</div>)}</div>
                  </div>
                  <div className="space-y-4 no-print">
                    <div className="bg-white border border-slate-200 rounded-3xl overflow-hidden"><button onClick={() => ipToggleSection("skills")} className="w-full flex justify-between items-center px-5 py-4 hover:bg-slate-50"><h3 className="font-black flex items-center gap-2"><Target size={15} className="text-emerald-600" /> Skills ({ipCompletedSkills(ipSelected)}/{(ipSelected.skill_targets || []).length})</h3>{ipExpandedSections.skills ? <ChevronUp size={15} /> : <ChevronDown size={15} />}</button>{ipExpandedSections.skills && (<div className="px-5 pb-5 space-y-2">{(ipSelected.skill_targets || []).map((s, i) => (<button key={i} onClick={() => ipToggleSkill(i)} className={`w-full flex items-center gap-2.5 p-2.5 rounded-xl text-left transition-colors ${s.achieved ? "bg-emerald-50 border border-emerald-200" : "bg-slate-100 hover:bg-blue-50"}`}>{s.achieved ? <CheckCircle size={16} className="text-emerald-600 shrink-0" /> : <Circle size={16} className="text-slate-400 shrink-0" />}<span className={`text-xs font-semibold ${s.achieved ? "text-emerald-700 line-through" : "text-slate-800"}`}>{s.skill}</span></button>))}</div>)}</div>
